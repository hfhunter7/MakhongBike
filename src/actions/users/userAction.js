...
export function update_account( id, data,dispatch,update_user ) {
    const user_storage = current_user_storage();
    const auth_token = user_storage.auth_token;

    fetch(defaultUrl + `/trainer/bankaccount/${id}`,
        {
            headers: {
                'Content-Type': 'application/json',
                'Authorization': createJwtFromToken('', auth_token),
        ...
        .then(function ( res ) {
            login_user_from_auth_token(auth_token, dispatch, update_user);
        });
}

export function add_bank( data, dispatch, update_user ) {
    const user_storage = current_user_storage();
    const auth_token = user_storage.auth_token;

    fetch(defaultUrl + '/trainer/bankaccount/',
        {
            headers: {
                'Content-Type': 'application/json',
                'Authorization': createJwtFromToken('', auth_token)
...
export function delete_bank_account( dispatch, id, update_user ) {
    const user_storage = current_user_storage();
    const auth_token = user_storage.auth_token;

    return fetch(defaultUrl + `/trainer/bankaccount/${id}`,
        {
            headers: {
                'Content-Type': 'application/json',
            },
